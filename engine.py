    ...
    def evaluate_pitcher_scenario(self, score_diff: int = None, is_enemy: bool = False, is_defense: bool = True, **kwargs) -> int:
        ...
        if is_defense:
            current_pitcher = getattr(self, 'our_pitcher', None) or (
                ...
                if hasattr(self, 'my_pitchers') and self.my_pitcher_idx < len(self.my_pitchers) 
                else None
            )
        else:
            current_pitcher = getattr(self, 'enemy_pitcher', None) or (
                self.enemy_pitchers[self.enemy_pitcher_idx] 
                if hasattr(self, 'enemy_pitchers') and self.enemy_pitcher_idx < len(self.enemy_pitchers) 
                else None
            )
