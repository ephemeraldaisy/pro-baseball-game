...
# 구종과 타자 예상 위치 및 투구 존 초기화
if "pitch_type" not in st.session_state: st.session_state.pitch_type = "직구"
if "pitch_zone" not in st.session_state: st.session_state.pitch_zone = 5 
...
def setup_half_inning():
    ...
    # ⚾ [투구 선행 장전 엔진] 사용자가 액션 버튼을 누르기 전, 미리 공을 결정하여 교대/반복 시 정보 고정
    ...
    if not current_is_our_turn:
        ...
        if enemy_pts == 0:
            if "defense_type" in locals() and defense_type == "KKK":
        ...
        if idx < 12:
            if st.session_state.is_home_team: 
                if st.session_state.away_inning_scores[idx] == "": st.session_state.away_inning_scores[idx] = enemy_pts
                else: st.session_state.away_inning_scores[idx] += enemy_pts
            else:
                if st.session_state.home_inning_scores[idx] == "": st.session_state.home_inning_scores[idx] = enemy_pts
                else: st.session_state.home_inning_scores[idx] += enemy_pts
...
# 🏃‍♂️ [대교정 완성판] 일반 도루 실패 시 주자 위치별 삭제 및 3루 단독 기습 홈스틸 우선순위 분기 엔진
def trigger_steal():
    ...
    # 🎯 [우선순위 엔진] 3루에 주자가 있어도 1루나 2루에 다른 주자가 묶여있다면 일반 진루 도루 작전을 우선 처리
    if st.session_state.base1 or st.session_state.base2:
        ...
        if random.random() < 0.60:  
            ...
            # ❌ [유령 주자 완전 차단] 도루 시도 루상에 부합하는 활성 주자를 정확히 판별하여 필드에서 삭제
            if st.session_state.base2:
                st.session_state.base2 = False
                st.session_state.game_log.append("❌ 포수의 칼 같은 송구! 2루 주자가 3루에서 태그 아웃되었습니다! (아웃 +1)")
            elif st.session_state.base1:
                st.session_state.base1 = False
                st.session_state.game_log.append("❌ 포수의 칼 같은 송구! 1루 주자가 2루에서 태그 아웃되었습니다! (아웃 +1)")
    ...
    # 🚨 타 베이스가 완전히 비어있고, 오직 '3루 주자 혼자만' 존재하는 틈새 상황에서 기습 홈스틸 강제 개막
...
# ==========================================
# 3. 타격 액션 및 진루 처리 (병살타 / 삼진 오심 200% 박멸 완료)
# ==========================================
def play_turn(user_choice):
    ...
    # 👀 3. 공 끝까지 거르기 (🚨 공 위치 기반 100% 팩트 칼판정 엔진 매칭 완료)
    elif user_choice == 3:
    ...
    if at_bat_result != "지속":
        ...
        if at_bat_result == "홈런":
            ...
            if st.session_state.base1 and st.session_state.out_count < 2 and random.random() < 0.40:
                ...
                st.session_state.base1 = False  # 🔥 병살타 발생 시 주자 강제 삭제 조치 고정
                ...
                st.session_state.out_count += 1
                st.session_state.base3 = False  # 희생플라이로 홈인했으므로 3루 주자 삭제
                st.session_state.our_score += 1
    ...
    if idx < 12:
        if st.session_state.is_home_team:
            ...
            if actual_gained_run > 0:
                if st.session_state.home_inning_scores[idx] == "": st.session_state.home_inning_scores[idx] = actual_gained_run
                else: st.session_state.home_inning_scores[idx] += actual_gained_run
        else:
            prev_score = sum([x for x in st.session_state.away_inning_scores if type(x) in [int, float]])
            actual_gained_run = st.session_state.our_score - prev_score
            if actual_gained_run > 0:
                if st.session_state.away_inning_scores[idx] == "": st.session_state.away_inning_scores[idx] = actual_gained_run
                else: st.session_state.away_inning_scores[idx] += actual_gained_run
                    
    # 공격 턴 9회말 끝내기 승리 요건 확인
    if (st.session_state.inning >= 9 and st.session_state.phase == "말" and st.session_state.is_home_team and st.session_state.our_score > st.session_state.enemy_score and not st.session_state.game_over):
        st.session_state.game_log.append(f"🎉 {st.session_state.inning}회말!!! 우리 팀 타석에서 짜릿한 '끝내기 득점'이 터지며 경기를 그대로 끝냅니다!!")
        end_game()

    # ⚾ 한 투구가 완료되었으므로 다음 투구를 선행 장전
    prepare_next_pitch()

def prepare_next_pitch():
    st.session_state.pitch_type = random.choice(["직구", "슬라이더", "체인지업", "커브", "포크볼", "스플리터"])
    if random.random() < 0.70:
        st.session_state.pitch_zone = random.randint(1, 9)
    else:
...
if st.session_state.show_matrix:
    ...
    try: st.dataframe(df.style.map(color_cells), use_container_width=True)
    except AttributeError: st.dataframe(df.style.applymap(color_cells), use_container_width=True)
