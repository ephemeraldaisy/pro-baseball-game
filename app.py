...
def play_turn(user_choice):
    ...
    log_msg = ""
    at_bat_result = "지속"  # 타석 결판 여부

    # =======================================================
    # 🟢 [투구수 시스템] 3번은 정직하게 1구씩, 1·2번은 현실적인 누적 투구수 적용
    # =======================================================
    if user_choice == 1:
    ...
    if user_choice == 1:
        ...
        if result == "HOMERUN": at_bat_result = "홈런"
            ...
            # ⚖️ [파울 규칙 이식] 2스트라이크 미만이면 스트라이크 추가!
            if st.session_state.strike < 2:
                st.session_state.strike += 1
                st.session_state.game_log.append(f"💥 작전[풀스윙]: 빗맞은 파울! 스트라이크 하나 추가되서 {st.session_state.strike}S 기라! (총 {st.session_state.enemy_total_pitches}구)")
            else:
                st.session_state.game_log.append(f"💥 작전[풀스윙]: 2S 이후 긴박한 끈질긴 파울 커트! 타석 유지! (총 {st.session_state.enemy_total_pitches}구)")
            return # 타석이 안 끝났으므로 다음 줄 계산 안 하고 즉시 함수 탈출!
    ...
    # =======================================================
        ...
        if result == "HIT": at_bat_result = "안타"
            ...
            if st.session_state.strike < 2:
                st.session_state.strike += 1
                st.session_state.game_log.append(f"💥 작전[밀어치기]: 3루 선상 아슬아슬한 파울! (현재 {st.session_state.strike}S) (총 {st.session_state.enemy_total_pitches}구)")
            else:
                st.session_state.game_log.append(f"💥 작전[밀어치기]: 커트! 투수를 괴롭히는 2S 이후 파울! (총 {st.session_state.enemy_total_pitches}구)")
            return # 타석 유지!

    # =======================================================
    # 👀 3. 공 끝까지 거르기 작전 (기존 거르기 맛 유지)
    # =======================================================
    elif user_choice == 3:
        result = random.choices(
            ["STRIKE", "BALL"], 
        ...
        if result == "STRIKE":
            ...
            if st.session_state.strike >= 3:
                st.session_state.game_log.append(f"⚡ 루킹 삼진 아웃!! (상대 투수 총 {st.session_state.enemy_total_pitches}구)")
                at_bat_result = "삼진"
        elif result == "BALL":
        ...
        if log_msg:
            st.session_state.game_log.append(f" 작전[공 거르기]: {log_msg}")

    # =======================================================
    # 🏃‍♂️ [통합 진루 및 타자 교체 엔진] 타석 결판 시만 작동
    # =======================================================
    if at_bat_result != "지속":
        st.session_state.strike = 0
        st.session_state.ball = 0
        ...
        if at_bat_result == "홈런":
            ...
            st.session_state.game_log.append(f"🔥 🎉 깡!!!!! {current_batter}번 타자 대형 {pts}점짜리 홈런 대폭발!!!!!!!! (총 {st.session_state.enemy_total_pitches}구)")
            st.session_state.base1 = st.session_state.base2 = st.session_state.base3 = False
            ...
            st.session_state.game_log.append(f"🌟 딱! {current_batter}번 타자의 안타! 주자 나갑니다! (총 {st.session_state.enemy_total_pitches}구)")

        elif at_bat_result == "아웃":
            st.session_state.out_count += 1
            st.session_state.game_log.append(f" Ah... {current_batter}번 타자 내야 땅볼 아웃입니다. (총 {st.session_state.enemy_total_pitches}구)")
            ...
            st.session_state.game_log.append(f"🚶‍♂️ {current_batter}번 타자 볼넷 출루! (총 {st.session_state.enemy_total_pitches}구)")
